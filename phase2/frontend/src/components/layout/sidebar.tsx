...
export default function Sidebar({ isCollapsed, toggleSidebar }: { isCollapsed: boolean; toggleSidebar: () => void }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:sticky top-0 left-0 h-full bg-slate-900 border-r border-slate-800 z-30 transition-all duration-300 ${
          isCollapsed ? 'w-16' : 'w-64'
        } ${isOpen ? 'w-64' : 'w-0 md:w-16'} md:w-64`}
      >
        <div className={`h-full flex flex-col ${isOpen ? 'block' : 'hidden md:block'}`}>
          {/* Logo */}
          <div className={`flex items-center border-b border-slate-800 p-4 ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
            {!isCollapsed && (
