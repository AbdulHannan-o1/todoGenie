...
def test_generate_recurring_task_daily(session: Session):
    original_due_date = datetime.now(UTC)
    original_task = Task(
        title="Daily Task",
        user_id=1,
        status="completed",
    ...
    assert new_task.due_date == (original_due_date + timedelta(days=1)).replace(tzinfo=None)
...
def test_generate_recurring_task_weekly(session: Session):
    ...
    assert new_task.due_date == (original_due_date + timedelta(weeks=1)).replace(tzinfo=None)
...
def test_generate_recurring_task_monthly(session: Session):
    ...
    assert new_task.due_date == (original_due_date + timedelta(days=30)).replace(tzinfo=None) # Simple monthly logic
