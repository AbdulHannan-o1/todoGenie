...
from src.services.ai_agent.tool_integration import execute_tool, get_available_tools
...
class AIAgentService:
    ...
    async def process_message(self,
        ...
        # All task operations are now handled by the AI model through MCP tools
        # The AI agent will let the model decide which tools to call based on user requests
        # No direct intent detection or handling is performed here
                ...
                try:
                    ...
                    if self.current_provider == "primary" and ("rate limit" in error_str.lower() or "api" in error_str.lower() or "connection" in error_str.lower() or "timeout" in error_str.lower() or "access has been paused" in error_str.lower()):
                        ...
                        try:
                            ...
                            # If backup also fails, return a graceful error response instead of raising
                            backup_error_str = str(backup_error)
                            final_response_text = f"Sorry, I'm currently experiencing technical difficulties and unable to process your request. Please try again later. ({backup_error_str})"
                    ...
                    # Check if this is a tool_use_failed error
                        ...
                        # For other errors, return a graceful response instead of raising
                        final_response_text = f"Sorry, I'm currently experiencing technical difficulties and unable to process your request. Please try again later. ({error_str})"
                        processing_time = time.time() - start_time
